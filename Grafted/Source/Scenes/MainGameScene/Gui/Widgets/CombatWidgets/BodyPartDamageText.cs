...
/// </summary>
public class BodyPartDamageTextRenderer
{
    ...
    public BodyPartDamageTextRenderer(IBodyPartLayout? layout, int nativeSize)
    {
        _layout = layout;
    ...
    public void AddDamageText(BodyPart? bodyPart, string text, Color color, float duration = 2f)
    {
        ...
        if (bodyPart != null && _layout != null)
        {
            var renderInfo = _layout.GetRenderInfo(bodyPart);
            ...
            else
            {
                ...
                position = new Vector2(_nativeSize / 2f, _nativeSize / 2f) + new Vector2(

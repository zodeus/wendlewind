...
public sealed class GrimoirePanel : EntityPanelBase
{
    public GrimoirePanel(BaseGui gui, Item item, EntityPanelProperties? properties = null) : base(gui, item, properties)
    {
        ...
        var flammables = DefRepository<ItemDef>.Defs.Where(d => d is { ItemType: ItemType.Flammable, CraftingProperties: not null }).ToList();
        ...
        tabs.AddTab("Flammables", new CraftingPanel("Prepare", flammables, Core.Context.PlayerPawn));
        Widgets.Add(tabs);
    }

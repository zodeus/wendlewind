...
namespace Grafted.Scenes.MainGameScene.Gui.Widgets.EntityWidgets.MedicinalPanels;

/// <summary>
/// Custom panel for Menders Mist that displays an infographic explaining how the healing works.
/// Shows that the mist heals bone, flesh, and skin while traveling through sockets to connected parts.
/// </summary>
[UsedImplicitly]
public sealed class MendersMistPanel : EntityPanelBase
{
    ...
    private static readonly Color NotHealedColor = new(80, 80, 80);    // Gray for not healed
    ...
    public MendersMistPanel(BaseGui gui, Item item, EntityPanelProperties? properties = null) 
        : base(gui, item, properties)
    {
        ...
        // ═══════════════════════════════════════════════════════════════════
        Widgets.Add(new HorizontalSeparator { Margin = new Thickness(0, 12, 0, 8) });

        var infoHeader = new Label(BaseContent.Styles.Label.Normal)
        {
            Text = "How It Works",
            TextColor = BaseContent.Colors.Text.Golden,
            Margin = new Thickness(0, 0, 0, 8)
        };
        Widgets.Add(infoHeader);

        // Create the infographic visualization
        Widgets.Add(CreateInfographic());

        // ═══════════════════════════════════════════════════════════════════
        // Legend Section
        // ═══════════════════════════════════════════════════════════════════
        Widgets.Add(CreateLegend());
    }

    private Widget CreateInfographic()
    {
        ...
        // Visual representation of body part healing flow
        // Shows: Torso → Arm → Hand with internal parts
    ...
    {
        // Create a visual showing a body part with its internal parts
        var container = new Panel
        {
    ...
    {
        // Show the flow: Part → Socket → External Part → Socket → External Part...
        var container = new Panel
        {
            Background = new SolidBrush(new Color(35, 35, 40)),
    ...
    private Widget CreateLegend()
    {
        ...
        content.Widgets.Add(new Label("small")
        {
            Text = $"Healing pool: {_item.GetStatValue(Defs.Stats.HealingValue)} HP total",

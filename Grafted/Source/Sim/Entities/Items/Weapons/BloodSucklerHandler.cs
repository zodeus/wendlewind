...
public class BloodSucklerHandler : WeaponHandler
{
    ...
    private void HealRandomDamagedParts(Pawn pawn, DamageRecord damageRecord)
    {
        ...
        var damagedParts = pawn.Body.AllParts
            .Where(p => p.HealthPercent < 0.99 && !p.IsDestroyed)
            .ToList();
        ...
        var partsToHeal = Core.Random.Next(MinPartsToHeal, Math.Min(MaxPartsToHeal + 1, damagedParts.Count + 1));
        ...
        {
            var healAmount = Core.Random.NextFloat(HealthRestoreMin, HealthRestoreMax);

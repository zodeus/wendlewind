        ...
        if (zone == Defs.Zones.MeatMarket) {
            ...
            Content = new VerticalStackPanel() {
                ...
                Widgets = {
                    ...
                    new Label(BaseContent.Styles.Label.Medium) { Text = $"Percent Complete: {Core.Sim.World.Zones[zone].PercentTraveled:P0}" },
                    new Label(BaseContent.Styles.Label.Medium) { Text = $"Boss Kill: {(Core.Sim.World.Zones[zone].IsComplete ? "Yes" : "No")}" },
                    new Label(BaseContent.Styles.Label.Medium) { Text = $"Kills: {Core.Sim.World.Zones[zone].TotalZoneKills}" },

...
namespace Wendlewind.Coroutines;
...
public class CoroutineManager /*: GlobalManager */ {
    ...
    public void Update(float deltaTime) {
        ...
        for (var i = 0; i < _unblockedCoroutines.Count; i++) {
            ...
            if (coroutine.WaitTimer > 0) {
                ...
                coroutine.WaitTimer -= /*coroutine.UseUnscaledDeltaTime ? Time.UnscaledDeltaTime :*/ deltaTime;
                _shouldRunNextFrame.Add(coroutine);
                continue;
            }
    ...
    bool TickCoroutine(CoroutineImpl coroutine) {
        ...
        if (coroutine.Enumerator.Current is float) {
            Log.Error("yield Coroutine.waitForSeconds instead of a float. Yielding a float will not work in a release build.");

...
namespace Wendlewind.Graphics.Textures;

/// <summary>
/// various utilities for creating textures on the fly. These can be pretty heavy on the CPU so it's best to not try to use them every frame.
/// </summary>
public static class TextureUtils {
    ...
    /// <summary>
    /// loads a Texture2D and pre-multiplies the alpha
    /// </summary>
    public static Texture2D? TextureFromStreamPreMultiplied(Stream stream) {
        var texture = Texture2D.FromStream(RenderHost.GraphicsDevice, stream);
    ...
    public static Texture2D CreateFlatHeightmap(Texture2D image, Color opaqueColor, Color transparentColor) {
        var resultTex = new Texture2D(RenderHost.GraphicsDevice, image.Width, image.Height, false, SurfaceFormat.Color);

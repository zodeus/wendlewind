...
{
    public const string CookieName = "ww_admin";
    ...
    {
        var material = Encoding.UTF8.GetBytes("wendlewind-admin:" + password);
    ...
    {
        var configured = Environment.GetEnvironmentVariable("WENDLEWIND_ADMIN_PASSWORD");
        if (!string.IsNullOrWhiteSpace(configured))
        {
            return new AdminAuth(configured.Trim())
            {
                StatusMessage = "password from WENDLEWIND_ADMIN_PASSWORD"
            };
        }
        ...
        if (environment.IsDevelopment())
        {
            ...
            {
                StatusMessage = "Development default password 'dev' (set WENDLEWIND_ADMIN_PASSWORD to override)"
            };
        }
        ...
        {
            StatusMessage = $"generated password {generated} (set WENDLEWIND_ADMIN_PASSWORD to persist)"
        };
    }

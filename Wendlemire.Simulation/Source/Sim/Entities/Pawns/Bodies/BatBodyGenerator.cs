...
public class BatBodyGenerator : IBodyGenerator
{
    public void Generate(Pawn pawn)
    {
        ...
        var head = pawn.Body.RootSocket.TryAttachPart(Defs.BodyParts.BatHead);
        ...
        head.Equipment[EquipmentSlotType.BuiltIn] = head.Context.Factory.CreateEntity<Item>(DefRepository<ItemDef>.GetByMoniker("BatFangs")!);
    ...
    private static void MakeClaw(BodyPart claw)
    {
        ...
        claw.Equipment[EquipmentSlotType.BuiltIn] = claw.Context.Factory.CreateEntity<Item>(DefRepository<ItemDef>.GetByMoniker("BatClaws")!);
    }
}

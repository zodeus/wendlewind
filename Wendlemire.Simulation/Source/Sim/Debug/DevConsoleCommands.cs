...
namespace Wendlemire.Sim.Debug;

/// <summary>
/// Parses and executes slash-commands against a live <see cref="GameContext"/>.
/// </summary>
public static class DevConsoleCommands
{
    public static string Execute(GameContext context, string input)
    {
        ...
        var parts = ParseCommand(input[1..]);
        if (parts.Length == 0)
        {
        ...
        {
            "help" => Help(args),
            "create-entity" or "spawn" or "give" => CreateEntity(context, args),
            "list-items" or "items" => ListItems(args),
            "list-pawns" or "pawns" => ListPawns(args),
            "clear" => "CLEAR",
            "heal" => Heal(context, args),
            "kill" => Kill(context),
            "complete-zone" => CompleteZone(context),
            "stats" => ShowStats(context),
            "tp" or "teleport" => Teleport(context, args),
            "fight" => Fight(context),
    ...
    private static string CreateEntity(GameContext context, string[] args)
    {
        if (args.Length < 1)
        {
        ...
        var item = context.Factory.CreateEntity<Item>(itemDef, count);
        return playerPawn.Inventory.TryAdd(item)
            ? $"Created {count}x {itemDef.Label} ({itemDef.Moniker})"
            : $"Created {itemDef.Label} but couldn't add to inventory (full?).";
    }
    ...
    private static string ListItems(string[] args)
    {
        ...
        if (items.Count == 0)
        {
            return filter != null ? $"No items matching '{filter}'" : "No items found.";
        }
    ...
    private static string ListPawns(string[] args)
    {
        ...
        if (pawns.Count == 0)
        {
            return filter != null ? $"No pawns matching '{filter}'" : "No pawns found.";
        }
    ...
    private static string ListZones(string[] args)
    {
        ...
        if (zones.Count == 0)
        {
            return filter != null ? $"No zones matching '{filter}'" : "No zones found.";
        }
    ...
    private static string Heal(GameContext context, string[] args)
    {
        var player = context.PlayerPawn;
        if (player == null)
        {
            return "Error: No player pawn found.";
        }
    ...
    private static string Kill(GameContext context)
    {
    ...
    private static string CompleteZone(GameContext context)
    {
        var zone = context.CurrentZone;
        if (zone == null)
        {
        ...
        context.World.ProgressTracker.OnZoneCompleted(zone);
        return $"Completed zone: {zone.ZoneDef.Label}";
    }

    private static string ShowStats(GameContext context)
    {
        var player = context.PlayerPawn;
        if (player == null)
        {
    ...
    private static string Teleport(GameContext context, string[] args)
    {
        if (args.Length < 1)
        {
        ...
        var zoneDef = DefRepository<ZoneDef>.GetByMoniker(moniker, raiseError: false)
                      ?? DefRepository<ZoneDef>.Defs.FirstOrDefault(d =>
                          d.Moniker.Contains(moniker, StringComparison.OrdinalIgnoreCase) ||
    ...
    private static string Fight(GameContext context)
    {
        var zone = context.CurrentZone;
        if (zone == null)
        {

...
public static class GenTypes
{
    ...
    public static List<string> ImpliedNamespaceNames {
        get {
            ...
            return _impliedNamespaceNames ??= AllTypes
                .Where(t => t.Namespace?.StartsWith("Wendlemire") == true)
    ...
    private static IEnumerable<Assembly> AllActiveAssemblies {
        get {
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                if (name != null && name.StartsWith("Wendlemire"))
                {
    ...
    {
        if (typeName != null && typeName.StartsWith("Wendlewind", StringComparison.Ordinal))
        {
            typeName = "Wendlemire" + typeName["Wendlewind".Length..];
        }

        if (namespaceIfAmbiguous != null && namespaceIfAmbiguous.StartsWith("Wendlewind", StringComparison.Ordinal))
        {
            namespaceIfAmbiguous = "Wendlemire" + namespaceIfAmbiguous["Wendlewind".Length..];
        }

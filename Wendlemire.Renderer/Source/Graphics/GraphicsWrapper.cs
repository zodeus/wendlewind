...
public class GraphicsWrapper {
    ...
    public GraphicsWrapper() {
        Batcher = new SpriteBatch(RenderHost.GraphicsDevice);
        //todo this should reference something better
        PixelTexture = new Sprite(RenderHost.Content.Load<Texture2D>("pixel.png"), 0, 0, 1, 1);
        RenderHost.PixelTexture = PixelTexture;
    }
    ...
    public static Texture2D CreateSingleColorTexture(int width, int height, Color color) {
        Texture2D texture = new(RenderHost.GraphicsDevice, width, height);
